...
1. **底线跨越（“过线即强”功能阈值 $Z \ge +1.5\sigma$ 的显式物理推导）**：
   ...
   * **显式数理推导**：设环境高熵背景扰动为高斯白噪声 $\xi(t) \sim \mathcal{N}(0, \sigma^2)$。根据信号检测论（Signal Detection Theory），若要求算子信号被下游控制器无失真捕捉的检测概率测度满足 $P_{\text{detect}} \ge 93.3\%$（错误率 $P_{\text{error}} \le 6.7\%$），则算子输出均值偏离群体分布必须满足 $Z = \frac{\mu - \mu_0}{\sigma} = \Phi^{-1}(0.933) = +1.50\sigma$。这精准对应于输入信号信噪比（SNR）下界：
     $$\text{SNR}_{\text{min}} = 10 \log_{10} \left( Z^2 \right) = 10 \log_{10} (2.25) \approx 3.52 \text{ dB}.$$
   若任意算子低于 $+1.5\sigma$，信号将在背景干扰中淹没，产生无盲区阻断物理漏洞。
