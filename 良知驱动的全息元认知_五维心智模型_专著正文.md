...
2. **相比传统特质心理学（大五 Big Five / HEXACO）的可计算增量与分层回归方程**：
   ...
   - **显式效应量预测**：理论预测在控制了大五全套特质（特别是开放性 Openness 与尽责性 Conscientiousness）后，第二层引入二阶算子（$\hat{\Omega}_{\text{meta}}, E_{\text{supp}}$）将产生显著的增量解释方差：
     ...
     这证明五维模型并非大五的重述，而是在 Non-IID 死锁边界上提供了不可归约的因果解释增量。
