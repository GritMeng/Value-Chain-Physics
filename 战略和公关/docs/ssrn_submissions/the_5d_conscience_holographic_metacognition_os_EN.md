...
```mermaid
graph TD
    classDef brain fill:#fdf,stroke:#a6a,stroke-width:2px;
    classDef control fill:#def,stroke:#68a,stroke-width:2px;

    Env[External Reality] -->|"Input Observation"| SN[2nd Dim: Hypersensitivity / Salience Net / Locus Coeruleus]
    SN -->|"Perceptual Residual Δ(t)"| MNS[3rd Dim: Sentient Depth / Limbic System / Mirror Neurons]
    SN -->|"Synaptic Gain Modulation ΓΔ"| dlPFC[4th Dim: Fluid Intelligence / dlPFC / Frontoparietal Net]
    
    MNS -->|"Visceral Experience Cache C_sentient"| dlPFC
    
    dlPFC -->|"Orthogonal Decomposition B ≤ 7"| vmPFC[1st Dim: Embodied Conscience / vmPFC / ACC / Insula]
    
    vmPFC -->|"Highest Bayesian Prior p(s) & Truncation E_supp"| Action[Active Inference Action]
    
    Action -->|"Environmental Feedback & Prediction Error"| SN
    
    dlPFC -->|"Gödelian Deadlock & Semantic Undecidability"| aPFC[5th Dim: Metacognition / aPFC / BA10 Area]
    aPFC -->|"Second-Order Meta-Audit"| dlPFC
    aPFC -->|"Evolution Functor Φ / Axiom Extension e_new"| vmPFC

    class SN,MNS,dlPFC,vmPFC,aPFC brain;
    class Env,Action control;
```
