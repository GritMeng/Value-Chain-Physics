...
graph TD
    ...
    Env[外部物理环境 / External Reality] -->|"输入观测 Input Observation"| SN[第二维：高敏感 Salience Network / 蓝斑核 LC]
    SN -->|"残差捕捉 Δ(t)"| MNS[第三维：极度感性 Limbic System / 镜像神经元 MNS]
    SN -->|"突触增益调制 ΓΔ"| dlPFC[第四维：流体智力 dlPFC / 额顶控制网]
    
    MNS -->|"经验与势能缓存 C_sentient"| dlPFC
    
    dlPFC -->|"正交降维求解 B ≤ 7"| vmPFC[第一维：具身良知 vmPFC / 前扣带回 ACC / 前岛叶 Insula]
    
    vmPFC -->|"最高阶先验 p(s) & 紧支撑截断 E_supp"| Action[动作执行 / Active Inference Action]
    
    Action -->|"环境反馈 & 预测误差"| SN
    
    dlPFC -->|"哥德尔死锁 & 语义不可判定"| aPFC[第五维：元认知 aPFC / BA10区]
    aPFC -->|"二阶 Meta-Bayesian 审计"| dlPFC
    aPFC -->|"进化函子 Φ (引入新基底 e_new)"| vmPFC
