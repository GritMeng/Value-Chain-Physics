...
作者建立了严密正交的学术单篇典藏体系（不含综合大包，包含 4 篇中文单篇与 4 篇英文单篇）：

### 🇨🇳 中文单篇学术专著/论文 (4 篇)
1. 📝 **[《系统与复杂性科学：秩序的生成、存续和进化》 (中文 72页主手稿正本)](./docs/System_and_Complexity_Science_Monograph_v2_CN.md)**
   - *定位与贡献*：系统科学与复杂性科学奠基巨著。确立生成、存续、进化三大公理，证明“观察者-边界双重性”与 $O(N!) \to O(N \log N)$ 算子降维。
2. 📝 **[《价值链管理物理学》 (中文 工业应用与算法主册)](./value-chain-physics-book.md)**
   - *定位与贡献*：详述工业级解算引擎（IPC）底层数据模型、执行算法与数据结构算法机制，给出千亿级离散制造网络高维非线性约束消纳的完整实证。
...
## 🏛️ GritMeng 钛媒体（TMTPost）已发表 14 篇核心思想长城专栏

归档了 GritMeng 在钛媒体（TMTPost）专栏公开发表的 14 篇重磅理论与实战解构长文：
