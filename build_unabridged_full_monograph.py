...
print("Writing FULL UNABRIDGED 4-Volume English LaTeX manuscript matching user's complete text...")

# Full unabridged LaTeX source code of all 4 volumes without summarize/condensation
full_unabridged_latex = r"""\documentclass[11pt,a4paper]{article}
...
Reductionism assumed linear additivity and static lead times, collapsing under factorial complexity $O(N!)$ in open complex giant systems.

\subsection{Chapter 2: Paradigm Shift and Calculable Work}
We formalize the five work operators:
\begin{enumerate}
...
\subsection{Chapter 3: Embodied Operators and Neural Topologies}
\begin{table}[htbp]
\centering
...
# Remove any accidental CJK non-ASCII characters to keep pdflatex 100% happy
def remove_non_ascii(text):
    return re.sub(r'[^\x00-\x7F]+', '', text)

cleaned_tex = remove_non_ascii(full_unabridged_latex)

with open(target_main_tex, "w", encoding="utf-8") as f:
    f.write(cleaned_tex)

print(f"Successfully generated FULL UNABRIDGED main.tex! File size: {len(cleaned_tex)} bytes")
