...
# Files configuration
tasks = [
    ("Paper_1_Formal_Proof_OCGS_Qian_Xuesen_EN_DoubleBlind.md", 
     "Paper_1_Formal_Proof_OCGS_Qian_Xuesen_EN_DoubleBlind.docx", 
     "Paper_1_Formal_Proof_OCGS_Qian_Xuesen_EN_DoubleBlind.pdf", True),
    ("IEEE_Submission_Title_Page.md", 
     "IEEE_Submission_Title_Page.docx", 
     "IEEE_Submission_Title_Page.pdf", True),
    ("IEEE_Submission_Cover_Letter.md", 
     "IEEE_Submission_Cover_Letter.docx", 
     "IEEE_Submission_Cover_Letter.pdf", True)
]

def preprocess_md(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    content = re.sub(r'!\[([^\]]*)\]\(file:///[^\)]*vcp_axioms_logic_chain_en\.png\)', r'![\1](vcp_axioms_logic_chain_en.png)', content)
...
def style_docx(docx_path, is_english=True):
    ...
    for p in doc.paragraphs:
        if p.style.name.startswith('Heading 1'):
            ...
            p.style.font.color.rgb = RGBColor(0x0F, 0x2C, 0x59)
...
def main():
    ...
    try:
        for md_name, docx_name, pdf_name, is_en in tasks:
            md_path = os.path.join(base_dir, md_name)
            docx_path = os.path.join(base_dir, docx_name)
            pdf_path = os.path.join(base_dir, pdf_name)
            
            temp_md = preprocess_md(md_path)
            convert_with_pandoc(temp_md, docx_path)
            style_docx(docx_path, is_english=is_en)
        ...
        word.Quit()
        print("IEEE submission package build completed!")
